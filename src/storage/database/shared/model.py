...
class RssTracking(Base):
    __tablename__ = 'rss_tracking'
    __table_args__ = (
        PrimaryKeyConstraint('id', name='rss_tracking_pkey'),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    link: Mapped[str] = mapped_column(Text, unique=True, nullable=False)
    title: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    processed_at: Mapped[datetime.datetime] = mapped_column(DateTime(True), server_default=text('now()'))
