...
class RSSSyncer:
    ...
    def get_feishu_access_token(self) -> Optional[str]:
        ...
        print("🔑 获取飞书访问令牌...")
        print(f"  App ID: {self.feishu_app_id}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.feishu_access_token = result.get("app_access_token")
                print(f"✅ 成功获取飞书访问令牌: {self.feishu_access_token[:20]}...")
                return self.feishu_access_token
            else:
                print(f"❌ 获取飞书令牌失败")
                print(f"  响应: {response.text}")
    ...
    def write_to_feishu(self, item: Dict) -> bool:
        ...
        try:
            ...
            # 尝试多种字段格式，以适应不同的飞书表格配置
            # 格式1: "消息链接"使用对象格式（URL类型）
            record = {
                "fields": {
                    "聊天记录": str(item["title"]),
                    "消息链接": {
                    ...
                    "时间": str(item["published"]),
                    "发送日期": str(item["description"])
                }
            }
            
            print(f"  写入URL: {url}")
            print(f"  记录内容: {json.dumps(record, ensure_ascii=False)}")
            ...
            if response.status_code == 200:
                ...
                response_alt = requests.post(
                    url,
                    headers=headers,
                    json={"records": [record_alt]},
                    timeout=30
                )
                
                print(f"  格式2状态码: {response_alt.status_code}")
                
                if response_alt.status_code == 200:
                    print(f"  ✅ 格式2写入成功")
                    return True
                else:
                    print(f"  ❌ 格式2也失败: {response_alt.text}")
                    return False
    ...
    def run(self) -> Dict:
        ...
        for i, item in enumerate(new_items, 1):
            print(f"\n[{i}/{len(new_items)}] 处理: {item['title'][:40]}...")
...
def main():
    """主函数"""
    print("\n")
    print("🎯 RSS监测与飞书同步 - GitHub Actions版（改进版）")
    print("\n")
